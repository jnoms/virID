...
Finally, the get_LCA.py script uses [ETE3](http://etetoolkit.org/docs/latest/tutorial/tutorial_ncbitaxonomy.html) to look up taxonomy information from a downloaded taxonomy database. When this script is first executed, it will automatically download a ~300MB taxonomy database to `~/.etetoolkit/taxa.sqlite`. One thing to note is that if your home directory is somewhere with slow I/O, you should make a symbolic link to somewhere faster prior to downloading the database, i.e. run `ln -s /faster/directory/etetoolkit ~/.etetoolkit` prior to running get_LCA.py. For reference, get_LCA.py script should run in about 40s on a BLAST output file of 500K lines.
