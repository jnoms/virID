...
## Prepare databases
This workflow requires three databases: 1) A DIAMOND database for search, 2) A nucleotide blast database for search, and 3) a contaminant nucleotide blast database (virID v2.0+). In anycase, you can activate the pipeline conda environment with `conda env create -f resouces/virID_environment.yml` so that you are making the DIAMOND and blast databases with the same version of DIAMOND and blast used by this pipeline.
